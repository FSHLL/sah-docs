...
<div class="custom-container tip"><p class="custom-container-title">Alias</p>
<p>A Lambda alias is a pointer to a function version that you can update.
When you deploy a new version, you can update the alias to use the new version, or split traffic between two versions.
See <a href="https://docs.aws.amazon.com/lambda/latest/dg/configuration-aliases.html" target="_blank" rel="noopener noreferrer">Create an alias for a Lambda function<ExternalLinkIcon/></a></p>
</div>
...
<details class="custom-container details"><summary>CloudFormationTemplate</summary>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">AWSTemplateFormatVersion</span><span class="token punctuation">:</span> <span class="token string">'2010-09-09'</span>
...
<p>As we can see we do not have aliases associated to our functions, the aliases work as pointers to specific versions of a function, this allows us to manage more easily our versions without the need to update the stack of that project as the <a href="https://www.serverless.com/framework/docs/providers/aws/cli-reference/rollback" target="_blank" rel="noopener noreferrer">serverless rollback<ExternalLinkIcon/></a> command currently does.</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>serverless rollback <span class="token parameter variable">--timestamp</span> timestamp
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>With SAM we can easily create these aliases in our serverless project in this way if we want to rollback to a previous version we just need to configure the alias to point to this.</p>
...
<ul>
<li>Create an <strong>&quot;ACTIVE&quot;</strong> alias, and using that alias in <strong>API Gateway</strong>, <strong>Event Sources</strong> and <strong>Event Rules</strong>.</li>
<li>Connect your deployments with the <strong>SAH application</strong>.</li>
</ul>
<p><a href="https://github.com/FSHLL/sah-plugin" target="_blank" rel="noopener noreferrer">GitHub Repository<ExternalLinkIcon/></a></p>
...
<p><a href="https://github.com/FSHLL/sah" target="_blank" rel="noopener noreferrer">GitHub Repository<ExternalLinkIcon/></a></p>
</div></template>
